...
def get_pagerduty_incidents():
    ...
    params = {'since': since_time_str, 'service_ids[]': Constant.SERVICE_ID, 'offset': 0, 'limit': 100}
...
print(len(recent_incidents))

if recent_incidents:
    for name in Constant.XIAN_ONCALL:
        for incident in recent_incidents:
            incident_id = incident['id']
            incident_summary = incident['summary']
            if name not in incident_summary:
                continue
                    ...
                    if log_entries:
                        ...
                        for log_entry in sorted_log_entries:
                            ...
                            if log_entry['type'] == 'resolve_log_entry':
                                resolve_timestamp = datetime.strptime(log_entry['created_at'], "%Y-%m-%dT%H:%M:%SZ")

                        if ack_timestamp:
                            ack_duration = ack_timestamp - created_timestamp
                            print(f"ACK Duration: {int(ack_duration.total_seconds() / 60)} min(s)\tIncident:{incident_summary}")
                        elif resolve_timestamp:
                            ack_duration = resolve_timestamp - created_timestamp
                            print(f"ACK Duration: {int(ack_duration.total_seconds() / 60)} min(s)\tIncident:{incident_summary}")
                        else:
                            print(f"Incident:{incident_summary} \tNo ACK\thttps://nexttrucking.pagerduty.com/incidents/{incident_id}")
                    else:
